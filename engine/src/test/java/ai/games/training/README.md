...
## Randomisation: Creating Diverse Endgames
...
However, for **training data diversity**, you can enable **randomisation** via `-Dendgame.randomise=true`. This makes the algorithm randomly select reverse moves at each level instead of following the same sequence.

### Effect of Randomisation

**Without randomisation (deterministic):**
```
...
### Why Randomisation Matters for Deep Levels

At deep levels (e.g., Level 100), without randomisation:
...
| `-Dendgame.randomise=true\|false` | false | Enable random reverse move selection for diverse endgames. Recommended for deep levels (50+). |
