...
```bash
cd engine
./gradlew test --tests "ai.games.analysis.GameTreeAnalysisTest.testExhaustiveGameTreeAnalysis_100Games_Quick"
```
...
```bash
cd engine
./gradlew test --tests "ai.games.analysis.GameTreeAnalysisTest.testExhaustiveGameTreeAnalysis_1000Games"
```
...
```bash
cd engine
./gradlew test --tests "ai.games.analysis.GameTreeAnalysisTest"
```

## Configuration

Edit the test method in `src/test/java/ai/games/analysis/GameTreeAnalysisTest.java`:
...
4. **Capture output**: Redirect to file for analysis
   ```bash
   ./gradlew test --tests "ai.games.analysis.GameTreeAnalysisTest" 2>&1 | tee results.txt
   ```
